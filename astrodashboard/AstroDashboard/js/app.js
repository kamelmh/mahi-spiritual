...
const AppState = {
    ...
    settings: JSON.parse(localStorage.getItem('settings')) || { name: 'Kamel M. Abdelghani', birthDate: '1996-03-06', birthTime: '14:00', birthLocation: 'El Bayadh, Algeria', fajrReminder: true, asrReminder: true, maghribReminder: true, ishaReminder: true }
};
...
function navigateTo(page){
    ...
    switch(page){
        ...
        case 'audio': if(window.AudioPlayer)AudioPlayer.init(); break;
...
async function updateTransitInfo(){
    ...
    if(typeof TransitEngine!=='undefined'){
        ...
        const report=TransitEngine.computeFullTransitReport(typeof chartData!=='undefined'?chartData:null);
        if(report.active.length>0){
            ...
            transitStr='No tight aspects active';
        }
    }
    ...
    ti.innerHTML=`
        ...
        <div class="transit-item"><strong>Natal Moon:</strong> Scorpio 5°50' (Anuradha)</div>
...
async function updateTransitAspectsWidget(){
    ...
    if(!container || typeof TransitEngine==='undefined') return;
    if(typeof chartData==='undefined' || !chartData){
        try{ const r=await fetch('data/chart.json'); chartData=await r.json(); }catch(e){ return; }
    }
    TransitEngine.renderTransitCard('transitAspectsWidget', chartData);
}
...
function initLearning(){
    const listEl=$('fileList'); if(!listEl) return;
    const files=[
        {name:'VEDIC_CHART_ACCURATE.md',category:'Astrology',desc:'Corrected sidereal positions (D1-D60)'},
        {name:'MAHI_ASTRO_QURANIC_INTEGRATION.md',category:'Spiritual',desc:'Full astro-Quranic system'},
        {name:'MAHI_DAILY_SPIRITUAL_PRACTICE.md',category:'Practice',desc:'Daily schedule and practices'},
        {name:'LUNAR_QURAN_RECITATION_METHODOLOGY.md',category:'Method',desc:'Imran Hosein 30-day lunar method'},
        {name:'PLUTO_NAKSHATRA_ANURADHA.md',category:'Astrology',desc:'Pluto in Anuradha Pada 2'},
        {name:'SATURN_RETROGRADE_PISCES.md',category:'Astrology',desc:'Saturn retrograde in Purva Bhadra'},
        {name:'LIFE_PURPOSE_DARMA_AXIS.md',category:'Spiritual',desc:'Pluto-Saturn dharma axis'},
        {name:'JOURNEY_MAP_2026.md',category:'Planning',desc:'Practical timeline for 2026'}
    ];
    listEl.innerHTML=files.map(f=>`<div style="display:flex;align-items:center;padding:10px;background:var(--bg-tertiary);border-radius:8px;margin-bottom:6px;border-left:3px solid var(--accent-blue)"><div style="width:180px;font-size:12px;color:var(--accent-gold);font-weight:600">${f.name}</div><div style="width:80px;font-size:11px;color:var(--accent-blue)">${f.category}</div><div style="flex:1;font-size:12px;color:var(--text-secondary)">${f.desc}</div></div>`).join('');
}
...
async function initSoul(){
    ...
    const sb=chartData.soulBlueprint; if(!sb) return;
    const sp=$('soulPurpose'),pl=$('pastLife'),ch=$('challenges'),gf=$('gifts');
    if(sp) sp.innerHTML=`<p style="color:var(--text-primary);line-height:1.6;font-size:15px">${sb.soulPurpose}</p><p style="color:var(--accent-gold);margin-top:8px;font-style:italic">${sb.mysticArchetype}</p>`;
    if(pl) pl.innerHTML=`<p style="color:var(--text-primary);line-height:1.6">${sb.pastLifeGifts}</p>`;
    if(ch) ch.innerHTML=`<p style="color:var(--text-primary);line-height:1.6">${sb.currentLifeChallenges}</p><p style="color:var(--accent-gold);margin-top:8px;font-size:13px"><strong>Karmic Debts:</strong> ${sb.karmicDebts}</p>`;
    if(gf) gf.innerHTML=`<p style="color:var(--text-primary);line-height:1.6">${sb.spiritualGifts}</p>`;
}

async function initSurahs(){
    const grid=$('surahGrid'); const search=$('surahSearch'); if(!grid) return;
    let verses=null;
    try{ const r=await fetch('data/verses.json'); verses=await r.json(); }catch(e){ return; }
    if(!verses||!verses.verses) return;
    function renderSurahs(filter=''){
        const f=filter.toLowerCase();
        const matched=verses.verses.filter(v=>!f||v.surahName.toLowerCase().includes(f)||v.name.toLowerCase().includes(f)||String(v.surah).includes(f));
        grid.innerHTML=matched.map(v=>`<div class="surah-card" onclick="navigateTo('verses')" style="background:var(--bg-tertiary);border:1px solid var(--border-color);border-radius:8px;padding:16px;cursor:pointer;transition:transform 0.2s" onmouseover="this.style.transform='scale(1.02)'" onmouseout="this.style.transform='scale(1)'"><div style="font-size:14px;color:var(--accent-gold);font-weight:600;margin-bottom:4px">Surah ${v.surah}</div><div style="font-size:16px;color:var(--text-primary);font-weight:600;margin-bottom:4px">${v.surahName}</div><div style="font-size:13px;color:var(--text-secondary)">${v.name}</div></div>`).join('');
    }
    renderSurahs();
...
async function initDestiny(){
    ...
    const dashas=chartData.dashas; if(!dashas) return;
    const timeline=$('dashaTimeline');
    const current=$('currentDasha');
    const timing=$('timingWindows');

    const allDashas=[
        {dasha:'Jupiter',start:'1996-03-06',end:'2011-03-06',lord:'Jupiter'},
        {dasha:'Saturn',start:'2011-03-06',end:'2014-09-06',lord:'Saturn'},
        {dasha:'Mercury',start:'2014-09-06',end:'2031-09-06',lord:'Mercury'},
        {dasha:'Ketu',start:'2031-09-06',end:'2038-09-06',lord:'Ketu'},
        {dasha:'Venus',start:'2038-09-06',end:'2058-09-06',lord:'Venus'},
        {dasha:'Sun',start:'2058-09-06',end:'2064-09-06',lord:'Sun'},
        {dasha:'Moon',start:'2064-09-06',end:'2074-09-06',lord:'Moon'},
        {dasha:'Mars',start:'2074-09-06',end:'2081-09-06',lord:'Mars'},
        {dasha:'Rahu',start:'2081-09-06',end:'2099-09-06',lord:'Rahu'}
    ];
    const now=new Date();
    const currentDasha=allDashas.find(d=>now>=new Date(d.start)&&now<new Date(d.end))||dashas.current;
    ...
    if(timeline){
        timeline.innerHTML=allDashas.map(d=>{
            ...
            return `<div style="display:flex;align-items:center;padding:10px;border-left:3px solid ${isActive?'var(--accent-gold)':isPast?'#555':'var(--accent-blue)'};margin-bottom:8px;background:${isActive?'rgba(255,215,0,0.1)':'var(--bg-tertiary)'};border-radius:0 8px 8px 0">
                ...
                <div style="flex:1;font-size:12px;color:var(--text-secondary)">${d.start} → ${d.end}</div>
    ...
    if(current){
        ...
        current.innerHTML=`<div style="padding:16px;background:var(--bg-tertiary);border-radius:8px;border-left:4px solid var(--accent-gold)">
            <div style="font-size:20px;font-weight:700;color:var(--accent-gold);margin-bottom:8px">${cd.dasha} Dasha</div>
            <div style="font-size:13px;color:var(--text-secondary);margin-bottom:8px">${cd.start} → ${cd.end}</div>
            <div style="font-size:14px;color:var(--text-primary);margin-bottom:8px"><strong>Theme:</strong> ${cd.theme}</div>
            <div style="font-size:13px;color:var(--accent-blue);font-style:italic;margin-bottom:8px">${cd.quranicConnection}</div>
            <div style="font-size:13px;color:var(--text-secondary)"><strong>Advice:</strong> ${cd.advice}</div>
            ${cd.subLord?`<div style="font-size:12px;color:var(--text-secondary);margin-top:8px">Sub-Lord: ${cd.subLord}</div>`:''}
        </div>`;
    }
    ...
    if(timing){
        const events=[
            ...
            {date:'2028-03-06',event:'Mercury Dasha ends → Ketu Dasha begins',type:'dasha'},
            {date:'2026-07-19',event:'Moon at personal manzil (Batn al-Hut)',type:'lunar'},
