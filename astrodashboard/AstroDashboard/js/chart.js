...
async function initChart(){
    ...
    createHouseList();
    createYogaList();
    if(typeof TransitEngine!=='undefined') TransitEngine.renderTransitCard('transitAspects',chartData);
}
...
async function loadChartData(){
    ...
    try {
        ...
        if (r.ok) {
            ...
            // Format raw API output to fit chartData schema if needed
...
function formatApiChart(raw) {
    ...
    if (raw.planets && typeof raw.planets === 'object') {
        ...
        return {
            name: raw.name || 'Kamel',
            ascendant: { sign: 'Gemini', degree: 14.5 },
            planets: planetsList,
            houses: housesList,
            yogas: raw.yogas || []
        };
...
function createYogaList(){
    ...
    t.innerHTML=chartData.yogas.map(y=>`<div class="yoga-item"><div class="yoga-name">${y.name}</div><div class="yoga-desc">${y.description}</div><div class="yoga-insight">${y.quranicInsight||''}</div></div>`).join('');
}
