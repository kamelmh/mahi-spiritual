...
def calculate_houses(year: int, month: int, day: int, hour: int = 12, minute: int = 0, latitude: float = 34.07, longitude: float = 1.33) -> Dict:
    """
    Calculate house cusps using Placidus system.
    latitude/longitude: El Bayadh, Algeria (34.07°N, 1.33°E)
    """
    t = ts.utc(year, month, day, hour, minute)

    try:
        from skyfield import almanac
        f = almanac.houses(eph, t, [latitude, longitude], 'P')
        house_cusps = f[0]

        houses = {}
        for i, cusp in enumerate(house_cusps[:12]):
            sign, degree = get_sign(cusp)
            houses[f"house_{i+1}"] = {
...
def calculate_full_chart(name: str, year: int, month: int, day: int, hour: int = 12, minute: int = 0, latitude: float = 34.07, longitude: float = 1.33) -> Dict:
    ...
    # Calculate houses (if time is known)
    houses = None
    if hour != 12 or minute != 0:
...
FAMILY_MEMBERS = {
    "Kamel": {
        ...
        "location": {"latitude": 34.07, "longitude": 1.33},
        "role": "Son (Subject)",
    },
    "Kheireddine": {
